...
const commandOptions: { [option: string]: CommandLineOption } = {
	...
	"--proxy": {
		...
		argument: "url",
		description:
			"Fallback proxy if the request cannot be resolved. e.g.: http://github.com",
	},
...
const logger = {
	...
	request: (
		...
		logger.log(
			...
			date,
			ip,
			color(request.method, ANSIColor.cyan),
			color(request.url, ANSIColor.cyan),
...
const server = new WebServer({
					...
					if (typeof staticPath === "string") {
						try {
							...
							if (stat.isDirectory() && fileStaticPath) {
								...
								const entriesStatic = (await fs.existsDir(fileStaticPath))
									? await fs.readDir(fileStaticPath)
									: [];
								for (const index of server.defaultPages) {
									if (
										!entries.includes(index) &&
										entriesStatic.includes(index)
									) {
	...
	maxAllowedContentLength:
		typeof argv["--max-length"] === "string"
			? parseInt(argv["--max-length"], 10)
			: undefined,
});
...
server.start().then(() => {
	...
	// 创建一个数组，用于存储本地网络的IP地址
	const networkAddresses = [
		`Local:   ${server.url}`, // 本地地址
	];
	...
	for (const interfaceName in networkInterfaces) {
		...
		for (const iface of interfaces) {
			if (iface.family === "IPv4" && !iface.internal) {
				// 去掉CIDR后缀 /32
				const ipAddress = iface.address.replace(/\/32$/, "");
				networkAddresses.push(`Network: http://${ipAddress}:${server.port}`);
			}
		}
	}

	// 在控制台中展示所有本地网络的IP地址
	console.info(networkAddresses.join("\n"), ANSIColor.green);
